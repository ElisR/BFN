...
def flow_old(
    ...
    *,
    ...
    return jax.vmap(
        lambda y_t: jnp.histogram(y_t, bins=bins, density=True, range=mu_range)
    )(ys)
...
def flow(
    ...
    *,
    ...
    ps = jnp.exp(
        -0.5 * jnp.square((mus - gamma_ts * x) / jnp.sqrt(gamma_ts * (1 - gamma_ts)))
    ) / jnp.sqrt(2 * jnp.pi * gamma_ts * (1 - gamma_ts))
...
def sample_mu(
    x: Float[Array, "*shape"], sigma_1: float, steps: int, *, key: Key
) -> tuple[Float[Array, "steps *shape"], Float[Array, "steps *shape"]]:
